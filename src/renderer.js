...
function aboutModalHTML() {
  const versions = [
    ...
    { v:'1.10.18', notes:['Toast-template structural rewrite — IG menu now matches the BD-ROM Interactive Graphics conventions observed across commercial authoring tools (the prior single-field tuning across 11 hardware iterations never produced visible buttons on the LG BP350)', 'NEW button state model: normal state is invisible (object_id_ref=0xFFFF) and page.defaultSelectedButtonIdRef=0xFFFF — the menu opens to a bare background and the first arrow-key press reveals the nearest button (intentional UX, matches how commercial standalone-player menus behave)', 'ONE bitmap object per button (used for both selected and activated state) instead of three', 'Removed WDS segment; segment order is now ICS → PDS → ODS… → END', 'Restored ODS decode budget ceil(w·h/90) chained from ICS.DTS (v1.10.16 had flattened it to a constant); corrected ICS PTS−DTS lead 11664→12012; IG PES DTS leading nibble 0x1→0x0', 'Two epoch_start display sets per menu (composition_number 0 and 1), continuity counter continuous across both', 'Forensic spec: docs/v11018_toast_template_spec.md; 208 unit tests pass; structural diff vs reference confirms display-set count, WDS absence, ICS lead, and DTS marker all match', 'ISO at ~/Desktop/v11018_test.iso (hardware test pending)'] },
