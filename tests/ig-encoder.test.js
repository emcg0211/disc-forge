...
console.log('\n=== 16: buildMenuDisplaySet button state model (v1.10.19) ===');
{
  ...
  function parseICSButtonIds(m2tsBuffer) {
    ...
    const IG_PID = 0x1400;
    let cur = null;
    const results = { buttonIds: [], defSelBtn: null, defValidBtns: [], objRefs: [] };
    ...
    function flushPes(chunks, hdrLen) {
      ...
      for (let bg = 0; bg < numBogs; bg++) {
        ...
        for (let bn = 0; bn < numBtns; bn++) {
          ...
          // Button fixed struct (35 bytes) offsets: normal_start@+17, sel_start@+23, act_start@+29.
          results.objRefs.push({
            normal: (body[po + 17] << 8) | body[po + 18],
            sel:    (body[po + 23] << 8) | body[po + 24],
            act:    (body[po + 29] << 8) | body[po + 30],
          });
          // numNavCmds is at offset 33 (after all fixed fields)
  ...
  // Count IG segments by type across all PES in the IG PID (one segment per PES).
  function countSegments(m2tsBuffer) {
    const IG_PID = 0x1400;
    let cur = null;
    ...
    function flush(chunks, hdrLen) {
      const full = Buffer.concat(chunks);
      if (full.length < 9 + hdrLen + 1) return;
      const seg = full.slice(9 + hdrLen);
      const t = seg[0];
      if      (t === 0x18) counts.ics++;
      else if (t === 0x14) counts.pds++;
